...
const HeaderButton = ({ base, on, alt, link, onClick, className, isActive: _isActive }: HeadrButtonProps) => {
  ...
  useEffect(() => {
    if (link) {
      setActive(router.pathname.startsWith(link));
    }
  }, [router.pathname]);
  ...
  return (
    <ButtonWrap onClick={buttonClick} className={className}>
      <img src={isActive ? on : base} alt={alt} title={alt} />
    </ButtonWrap>
  );
};
