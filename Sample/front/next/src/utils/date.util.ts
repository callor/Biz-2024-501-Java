...
export class DateUtil {
  ...
  private static localeWeek = {
    ko: ['첫 번째', '두 번째', '세 번째', '네 번째', '마지막', '마지막'],
  };
  static setDate(
    date: Date,
    option: {
  ...
  static format(date: Date | string, format: string) {
    if (date instanceof Date) {
      return dateFns.format(date, format, { locale: ko });
    } else {
      return dateFns.format(new Date(date), format, { locale: ko });
    }
  }
  ...
  static distanceByToday(date: Date) {
    return dateFns.formatDistanceToNow(date, { locale: ko });
  }
  /**
   * @example
  ...
  static getLastWeekDay(date: Date) {
    return dateFns.set(dateFns.lastDayOfWeek(date), { hours: 23, minutes: 59, seconds: 59, milliseconds: 999 });
  }
  static copyHourMinute(date: Date, copyDate: Date) {
    const hours = copyDate.getHours();
    const minutes = copyDate.getMinutes();
    return dateFns.set(date, { hours, minutes });
  }
  ...
  static diffMonth(baseDate: Date, compareDate: Date) {
    return dateFns.differenceInCalendarMonths(baseDate, compareDate);
  }
  ...
  static getCalendarLastDay(date: Date) {
    return dateFns.set(dateFns.lastDayOfWeek(dateFns.endOfMonth(date)), {
      hours: 23,
      minutes: 59,
      seconds: 59,
      milliseconds: 999,
  ...
  static getWeekDates(date: Date, month?: Date) {
    return this.isInvalidDate(date)
      ? []
      : dateFns
          .eachDayOfInterval({
            start: dateFns.startOfWeek(date),
            end: dateFns.lastDayOfWeek(date),
          })
          .map((val) => ({
            day: this.format(val, 'd'),
            isEmpty: !dateFns.isSameMonth(val, month),
            value: val,
            ymd: this.format(val, 'yyyyMMdd'),
          }));
  }
}
