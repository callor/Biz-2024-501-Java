...
const createClient = async () => {
  const zipPath = path.resolve(config.db.clientPath, '..', `${config.os}-oracle-client.zip`);
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(zipPath)
      ...
      .on('close', () => {
        ...
        const files = fs.readdirSync(path.resolve(config.db.clientPath, clientDir.name));
        files.forEach((file) =>
          fs.renameSync(
