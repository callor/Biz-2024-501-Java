...
export class CrawlingBiz {
  ...
  async getYoutubePlayList({ channelId, cnt = 10 }: PlayListDTO) {
    ...
    // 현재 한경 유튜브 영상이 갱신이 없는 상태이므로 좀비제거를 위해 날짜 고정
    const crawlingDt = '20240731';
  ...
  async crawlingChannel({ channelId, cnt }: { channelId: string; cnt: number }) {
    const playListSelector =
      'ytd-two-column-browse-results-renderer > #primary #contents #content';
      ...
      ({ cnt, playListSelector }) => {
        ...
        if (playList.length > 0) {
          ...
          for (let i = 0; i < cnt; i++) {
            ...
            const a = playList[i].querySelector('#video-title-link') as HTMLAnchorElement;
